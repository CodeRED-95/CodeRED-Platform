...
export function adaptAgency(raw: RawAgency): Agency {
  ...
  return {
    ...
    category: readString(raw, 'classification_category') ?? readString(classification, 'category') ?? readString(classification, 'tamano'),
    ...
    isOperationsCenter: readBoolean(raw, 'is_operations_center') || readBoolean(raw, 'centro_operaciones'),
    terrestrialText: readString(raw, 'texto_chosen_terrestre') ?? readString(raw, 'chosen_terrestre'),
