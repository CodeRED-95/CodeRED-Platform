...
export function loadConfig(env = process.env) {
    const req = (k) => { const v = env[k]; if (!v)
        throw new AgentError('Missing required configuration: ' + k, 'CONFIG_MISSING'); return v; };
    const key = req('CODERED_AGENT_ENCRYPTION_KEY');
    if (Buffer.from(key, 'base64').length < 32 && key.length < 32)
