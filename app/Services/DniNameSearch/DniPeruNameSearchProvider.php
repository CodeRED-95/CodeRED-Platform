...
final class DniPeruNameSearchProvider implements DniNameSearchProviderInterface
{
    ...
    private function discoverForm(string $html): ?array
    {
        ...
        foreach ($xpath->query('//form') ?: [] as $form) {
            ...
            foreach ($xpath->query('.//input', $form) ?: [] as $input) {
                ...
                if ($name === '' || in_array($type, ['submit', 'button', 'reset', 'file'], true)) {
                    continue;
                }
            ...
            if ($action === '') {
                $action = (string) config('dni.name_search.providers.dniperu.url');
            } elseif (! str_starts_with($action, 'http')) {
                $action = $this->resolveUrl((string) config('dni.name_search.providers.dniperu.url'), $action);
            }

            // 'field_names', no compact('fieldNames'): buildPayload() lee
            // $form['field_names'] y compact habria producido 'fieldNames'. El
            // acceso a la clave inexistente lanzaba un warning que Laravel
            // convierte en ErrorException, y el catch (Throwable) de search()
            // lo enmascaraba como 'provider_unavailable', de modo que el
            // proveedor fallaba en TODA consulta sin dejar rastro del motivo.
            return ['method' => $method, 'action' => $action, 'inputs' => $inputs, 'field_names' => $fieldNames];
        }
    ...
    private function resolveFieldNames(DOMElement $form, DOMXPath $xpath, array $inputs): ?array
    {
        ...
        if (count($textInputs) < 3) {
            return null;
        }

        $labels = [];
        foreach ($xpath->query('.//label', $form) ?: [] as $label) {
            // Explicito y no `?->nodeValue`: los stubs de DOM declaran que
            // getNamedItem() devuelve DOMNode, asi que PHPStan da el nullsafe
            // por innecesario, pero en runtime devuelve null cuando el atributo
            // no existe -un <label> sin `for` es HTML perfectamente valido- y
            // quitar el nullsafe a secas seria un fatal.
            $forAttribute = $label->attributes?->getNamedItem('for');
            $for = trim($forAttribute instanceof DOMNode ? (string) $forAttribute->nodeValue : '');
        ...
        foreach ($textInputs as $input) {
            ...
            if (str_contains($hay, 'materno')) {
                $mapped['materno'] = $input['name'];
            } elseif (str_contains($hay, 'paterno')) {
                $mapped['paterno'] = $input['name'];
            } elseif (str_contains($hay, 'nombre')) {
                $mapped['nombres'] = $input['name'];
            }
        }
        if (count($mapped) === 3) {
            return ['nombres' => $mapped['nombres'], 'paterno' => $mapped['paterno'], 'materno' => $mapped['materno']];
        }
    ...
    private function parseMatches(string $html): array
    {
        ...
        foreach ($configured as $selector) {
            if (str_starts_with((string) $selector, '//')) {
        ...
        if ($nodes === []) {
            $nodes = iterator_to_array($xpath->query('//table//tr') ?: []);
        }
        foreach ($nodes as $node) {
            ...
            if ($node instanceof DOMElement) {
                ...
                foreach ($node->getElementsByTagName('td') as $cell) {
                    ...
                    if (is_string($cellText) && $cellText !== '') {
                        $cells[] = $cellText;
                    }
                }
                if (count($cells) >= 4) {
                    ...
                    foreach ($cells as $index => $cell) {
                        if (preg_match('/(?<!\d)'.preg_quote($dni, '/').' (?!\d)/', $cell) || trim($cell) === $dni) {
                            $dniIndex = $index;
                            break;
                        }
                    }
                    if ($dniIndex !== null) {
                        ...
                        if (count($remaining) >= 3) {
                            $name = ['nombres' => $remaining[0], 'paterno' => $remaining[1], 'materno' => $remaining[2]];
                        }
                    }
                }
            }
