...
class IntegrationProtocolService
{
    ...
    public function registerDiscovery(Integration $integration, array $document, ?string $ip = null, ?string $userAgent = null): void
    {
        ...
        $integration->forceFill(['hostname' => $document['hostname'] ?? $integration->hostname, 'instance_url' => $document['instance_url'] ?? $integration->instance_url, 'environment' => $document['environment'] ?? $integration->environment, 'last_ip' => $ip, 'protocol_version' => $document['protocol_version'] ?? $integration->protocol_version, 'connector_version' => $document['connector_version'] ?? $integration->connector_version, 'n8n_version' => $document['n8n_version'] ?? $integration->n8n_version, 'version' => $document['n8n_version'] ?? $document['version'] ?? $integration->version, 'running_workflows' => $document['workflow_count'] ?? $document['workflows_count'] ?? $integration->running_workflows])->save();
