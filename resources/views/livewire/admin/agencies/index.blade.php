            ...
            @if(auth()->user()->hasPermission('agencies.export'))
                <div class="relative" x-data="{ open: false }">
                    ...
                    <div x-cloak x-show="open" x-on:click.outside="open = false" class="layer-popover absolute right-0 mt-2 w-72 space-y-1 rounded-[var(--radius-card)] border border-[color:var(--color-border)] bg-[color:var(--color-surface)] p-2 shadow-2xl">
                        <a href="{{ $filteredExportUrl }}" class="focus-ring block rounded-[var(--radius-control)] px-3 py-2 text-sm text-[color:var(--color-text-primary)] transition hover:bg-white/5">Exportar resultados filtrados (.json)</a>
                        <a href="{{ $allExportUrl }}" class="focus-ring block rounded-[var(--radius-control)] px-3 py-2 text-sm text-[color:var(--color-text-primary)] transition hover:bg-white/5">Exportar todas las agencias (.json)</a>
                        @if(auth()->user()->hasPermission('agencies.backup.create'))
                            <button type="button" wire:click="createBackup" class="focus-ring block w-full rounded-[var(--radius-control)] px-3 py-2 text-left text-sm text-[color:var(--color-text-primary)] transition hover:bg-white/5">Crear copia de seguridad</button>
                        @endif
                        @if(auth()->user()->hasPermission('agencies.backup.view'))
                            <a href="{{ route('admin.agencies.backups.index') }}" class="focus-ring block rounded-[var(--radius-control)] px-3 py-2 text-sm text-[color:var(--color-text-primary)] transition hover:bg-white/5">Ver copias de seguridad</a>
                        @endif
                    </div>
                </div>
            @endif
        ...
        <tbody class="divide-y divide-white/5">
            @forelse ($agencies as $agency)
                <tr @class(['transition hover:bg-white/5', 'opacity-70' => $agency->trashed()])>
                    ...
                    <td class="px-5 py-4">
                        @if($agency->map_url)
                            <x-ui.button href="{{ $agency->map_url }}" size="sm" variant="link" target="_blank" rel="noopener">Abrir mapa</x-ui.button>
                        @endif
                    </td>
                    <td class="px-5 py-4">
                        <div class="flex flex-wrap gap-2">
