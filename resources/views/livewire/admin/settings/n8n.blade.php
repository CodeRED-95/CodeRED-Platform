<div class="space-y-6">
    ...
    <div class="grid gap-4 xl:grid-cols-2">
        @forelse($integrations as $integration)
            @php($online = $integration->isOnline())
            <x-ui.card>
                <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                        <div class="flex flex-wrap items-center gap-2"><h2 class="text-xl font-semibold">{{ $integration->instance_name }}</h2><x-ui.badge :tone="$online ? 'success' : 'danger'">{{ $online ? 'Conectado' : 'Desconectado' }}</x-ui.badge><x-ui.badge tone="info">{{ $integration->version ?? 'Sin versión' }}</x-ui.badge></div>
                ...
                <div class="mt-5 grid gap-3 md:grid-cols-4"><x-ui.stat-card label="Heartbeat" :value="$integration->last_seen_at ? max(0, $integration->last_seen_at->diffInSeconds()) : 0" description="segundos" /><x-ui.stat-card label="Workflows" :value="$integration->running_workflows ?? 0" /><x-ui.stat-card label="Capabilities" :value="$integration->capabilities_count" /><x-ui.stat-card label="Latency" :value="$integration->latency_ms ?? 0" description="ms" /></div>
