<div class="space-y-6">
    ...
    <div class="grid gap-4 xl:grid-cols-2">
        @forelse($integrations as $integration)
            ...
            @php($statusTone = match ($connectionStatus) { 'connected' => 'success', 'degraded' => 'warning', 'unpaired' => 'warning', 'revoked' => 'danger', default => 'danger' })
            <x-ui.card>
                ...
                <div class="mt-5 grid gap-3 md:grid-cols-4"><x-ui.stat-card label="Ultimo heartbeat" :value="$integration->last_seen_at ? max(0, $integration->last_seen_at->diffInSeconds()) : 'sin confirmar'" description="segundos" /><x-ui.stat-card label="Ultimo discovery" :value="$integration->capabilities_count > 0 ? 'publicado' : 'sin discovery'" /><x-ui.stat-card label="Capabilities" :value="$integration->capabilities_count" /><x-ui.stat-card label="Latency" :value="$integration->latency_ms ?? 0" description="ms" /></div>
