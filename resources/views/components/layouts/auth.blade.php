...
<main class="auth-shell grid min-h-dvh w-full grid-cols-1 overflow-hidden lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
    ...
    >
        ...
        <div class="auth-promo-inner relative z-10 flex w-full min-w-0 max-h-[calc(100dvh-3rem)] flex-col justify-center gap-5 xl:gap-6">
            <div class="min-w-0">
                {{ $promo ?? '' }}
            </div>
        </div>
    ...
    <section class="auth-form-shell flex min-h-dvh min-w-0 items-center justify-center px-4 py-6 sm:px-6 lg:px-8">
        <div class="auth-form w-full max-w-[30rem] min-w-0">
            {{ $slot }}
        </div>
    </section>
</main>
