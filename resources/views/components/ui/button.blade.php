...
@php
    $base = 'relative inline-flex items-center justify-center gap-2 rounded-[var(--radius-control)] font-medium transition focus-ring disabled:cursor-not-allowed disabled:opacity-50';
    $sizes = [
...
@else
    <button type="{{ $attributes->get('type', $type) }}" @disabled($disabled || $loading) @if($loadingTarget) wire:loading.attr="disabled" wire:target="{{ $loadingTarget }}" @endif {{ $attributes->except('type')->merge(['class' => $classes]) }}>
        @if ($loadingTarget)
